...
import {RADIUS,CENTER,normalAt,bigLakeNormal} from './navigation.js';
export function buildWater(world,large=false){
 const pos=[],uv=[],indices=[],rings=large?48:32,segments=large?128:96;const cx=large?0:-29,cz=large?0:-15,rx=large?110:24,rz=large?90:32;
 for(let j=0;j<=rings;j++)for(let i=0;i<=segments;i++){const a=i/segments*Math.PI*2,r=j/rings,x=cx+Math.cos(a)*rx*r,z=cz+Math.sin(a)*rz*r;pos.push(...(large?bigLakeNormal(x,z):normalAt(x,z)).multiplyScalar(RADIUS-.24).add(CENTER).toArray());uv.push(x,z);}
 ...
 const uniforms={shape:{value:new T.Vector4(cx,cz,rx,rz)},large:{value:large?1:0},shadeDirection:{value:new T.Vector3(0,1,0)},time:{value:0},center:{value:CENTER.clone()},swimmer:{value:new T.Vector2(1000,1000)},wake:{value:0},mist:{value:new T.Color('#ced8c8')}};
 ...
 fragmentShader:`uniform vec4 shape;uniform float large;uniform vec3 shadeDirection;uniform float time;uniform vec3 center;uniform vec2 swimmer;uniform float wake;uniform vec3 mist;varying vec2 lake;varying vec3 worldPosition;
 void main(){float r=length((lake-shape.xy)/shape.zw);float island=distance(lake,vec2(35.,8.));if(large>.5&&island<17.)discard;float depth=smoothstep(0.,large>.5?.24:.65,1.-r);if(large>.5)depth*=smoothstep(17.,25.,island);vec3 N=normalize(cross(dFdx(worldPosition),dFdy(worldPosition)));if(dot(N,center-worldPosition)<0.)N=-N;vec3 V=normalize(cameraPosition-worldPosition),L=normalize(center-worldPosition),H=normalize(V+L);float fresnel=pow(1.-max(0.,dot(N,V)),3.);float spec=pow(max(0.,dot(N,H)),160.);
 ...
 float daylight=1.-smoothstep(-.16,.16,dot(normalize(worldPosition-center),shadeDirection));color*=mix(vec3(.12,.22,.4),vec3(1.),daylight);float distanceToEye=distance(cameraPosition,worldPosition);float fog=1.-exp(-pow(distanceToEye*.00205,2.));color=mix(color,mist,fog);gl_FragColor=vec4(color,mix(.5,.9,depth));
