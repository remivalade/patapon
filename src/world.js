...
function updateSunEffects(t) {
  ...
  glowUniforms.strength.value =
    ...
    (0.1 +
      0.9 * daylightAt(camera.position.clone().sub(CENTER).normalize(), dayNight.direction.value));
...
const playerShadow = mesh(
  ...
  new T.MeshBasicMaterial({ color: '#324735', transparent: true, opacity: 0.2, depthWrite: false }),
  world,
);
...
function getContext() {
  ...
  if (Math.hypot(local.x, local.z) < 13 && Math.abs(local.y) < 5)
    return {
      type: 'call',
      text: expansion.lift.moving ? 'En route…' : expansion.lift.height > 1 ? 'Appeler' : 'Monter',
      disabled: expansion.lift.moving,
    };
  ...
  if (!climb) {
    if (riding) {
      ...
      if (
        ...
        const up = activeUp(),
          angle = Math.atan2(up.dot(forward.clone().cross(bikeHeading)), forward.dot(bikeHeading));
      ...
      if (Math.hypot(motion.side, motion.ahead) > 0.001) moveSurface(motion.side, motion.ahead, dt);
...
function animate() {
  ...
  if (mode === 'intro') {
    ...
    pitch =
      u < 3 ? 0.12 + 1.26 * smooth(u / 3) : u < 4.2 ? 1.38 : 1.38 - 1.26 * smooth((u - 4.2) / 2.8);
    updateCamera(dt);
    if (u >= 7) beginWalk();
  } else move(dt);
  if (world.visible) {
