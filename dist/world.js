...
let bikeHeading=new T.Vector3(0,0,1),brakeHeld=false,lastLookTime=-100;
...
function clearInput(){brakeHeld=false;drive.reset();walkHandling.reset();keys.clear();joy.x=joy.y=0;joy.id=null;lookId=null;$('knob').style.transform='';const stickElement=$('stick');stickElement.style.left='';stickElement.style.top='';stickElement.style.bottom='';}
...
function updateHud(){const c=getContext();$('action').hidden=!c;$('action').disabled=!!c?.disabled;if(c)$('action-label').textContent=c.text;$('brake').hidden=!riding;$('jump').hidden=riding||swimming||!!(climb&&expansion.lift.moving);}
...
$('jump').addEventListener('pointerdown',e=>{e.preventDefault();jump();});$('jump').addEventListener('click',e=>{if(e.detail===0)jump();});$('brake').addEventListener('pointerdown',e=>{e.preventDefault();brakeHeld=true;$('brake').setPointerCapture(e.pointerId);});for(const type of ['pointerup','pointercancel','lostpointercapture'])$('brake').addEventListener(type,()=>{brakeHeld=false;});$('action').addEventListener('click',contextAction);$('home').addEventListener('click',resetHome);
...
function startStick(e,floating=false){if(joy.id!==null)return;if(mode==='reveal')beginWalk();e.preventDefault();joy.id=e.pointerId;stickMoved=false;
...
renderer.domElement.addEventListener('pointerdown',e=>{if(mode==='reveal')beginWalk();if(mode!=='walk')return;if(e.clientX<innerWidth*.44&&e.clientY>innerHeight*.42){startStick(e,true);renderer.domElement.setPointerCapture(e.pointerId);return;}if(lookId!==null)return;lookId=e.pointerId;lookMoved=false;lastX=downX=e.clientX;lastY=downY=e.clientY;downAt=performance.now();lastLookTime=clock.elapsedTime;renderer.domElement.setPointerCapture(e.pointerId);});
renderer.domElement.addEventListener('pointermove',e=>{if(e.pointerId===joy.id){updateJoy(e);return;}if(e.pointerId!==lookId)return;if(!lookMoved&&Math.hypot(e.clientX-downX,e.clientY-downY)<8)return;lookMoved=true;const sensitivity=Math.PI*.65/Math.min(innerWidth,innerHeight);turnView(-(e.clientX-lastX)*sensitivity);pitch=T.MathUtils.clamp(pitch-(e.clientY-lastY)*sensitivity,-.65,1.54);lastX=e.clientX;lastY=e.clientY;lastLookTime=clock.elapsedTime;$('lookhint').style.opacity='0';});
...
if(lookId===null&&clock.elapsedTime-lastLookTime>1.4&&Math.abs(drive.speed)>3){const up=activeUp(),angle=Math.atan2(up.dot(forward.clone().cross(bikeHeading)),forward.dot(bikeHeading));forward.applyAxisAngle(up,angle*(1-Math.exp(-dt*1.8))).normalize();}
