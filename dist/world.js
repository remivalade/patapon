...
const raycaster=new T.Raycaster(),pointer=new T.Vector2();let lookId=null,lastX=0,lastY=0,downX=0,downY=0,downAt=0;
...
function bikeAction(){if(climb||swimming)return;if(riding){const right=forward.clone().cross(activeUp()).normalize();for(let i=0;i<8;i++){const offset=right.clone().applyAxisAngle(activeUp(),i*Math.PI/4);const n=navNormal.clone().multiplyScalar(RADIUS).addScaledVector(offset,4).normalize();const resolved=collisions.resolve(n,.65,0);if(!surfaceBlocked(resolved,.6)&&lakeRadius(resolved)>1.04){navNormal=resolved;forward.projectOnPlane(navNormal).normalize();riding=false;jumpHeight=jumpVelocity=0;player.position.copy(surface(navNormal));return;}}return;}
 if(player.position.distanceTo(expansion.bike.position)>8)return;riding=true;navNormal=expansion.bike.position.clone().sub(CENTER).normalize();forward.set(0,0,1).applyQuaternion(expansion.bike.quaternion).projectOnPlane(navNormal).normalize();jumpHeight=jumpVelocity=0;}
...
function getContext(){if(riding)return{type:'bike',text:'Descendre'};if(climb)return{type:'lift',text:expansion.lift.moving?'En route…':climb.onPlatform?(expansion.lift.height<10?'Monter':'Descendre'):'Appeler',disabled:expansion.lift.moving};if(swimming)return null;
...
function updateHud(){const c=getContext();$('action').hidden=!c;$('action').disabled=!!c?.disabled;if(c)$('action-label').textContent=c.text;$('jump').hidden=riding||swimming||!!(climb&&expansion.lift.moving);}
...
$('jump').addEventListener('click',jump);$('action').addEventListener('click',contextAction);$('home').addEventListener('click',resetHome);
...
const stick=$('stick');stick.addEventListener('pointerdown',e=>{if(joy.id!==null)return;if(mode==='reveal')beginWalk();e.preventDefault();joy.id=e.pointerId;stick.setPointerCapture(e.pointerId);updateJoy(e);});
function updateJoy(e){const r=stick.getBoundingClientRect();let x=e.clientX-r.left-r.width/2,y=e.clientY-r.top-r.height/2;const d=Math.hypot(x,y),limit=37;if(d>limit){x*=limit/d;y*=limit/d;}joy.x=x/limit;joy.y=y/limit;$('knob').style.transform=`translate(${x}px,${y}px)`;}
stick.addEventListener('pointermove',e=>{if(e.pointerId===joy.id)updateJoy(e);});for(const evt of ['pointerup','pointercancel','lostpointercapture'])stick.addEventListener(evt,e=>{if(e.pointerId===joy.id){joy.x=joy.y=0;joy.id=null;$('knob').style.transform='';}});
renderer.domElement.addEventListener('pointerdown',e=>{if(mode==='reveal')beginWalk();if(mode!=='walk'||lookId!==null)return;lookId=e.pointerId;lastX=downX=e.clientX;lastY=downY=e.clientY;downAt=performance.now();renderer.domElement.setPointerCapture(e.pointerId);});
renderer.domElement.addEventListener('pointermove',e=>{if(e.pointerId!==lookId)return;turnView(-(e.clientX-lastX)*.005);pitch=T.MathUtils.clamp(pitch+(e.clientY-lastY)*.005,-.65,1.54);lastX=e.clientX;lastY=e.clientY;$('lookhint').style.opacity='0';});
renderer.domElement.addEventListener('pointerup',e=>{if(e.pointerId!==lookId)return;if(Math.hypot(e.clientX-downX,e.clientY-downY)<12&&performance.now()-downAt<500)interactAt(e.clientX,e.clientY);lookId=null;});
for(const evt of ['pointercancel','lostpointercapture'])renderer.domElement.addEventListener(evt,e=>{if(lookId===e.pointerId)lookId=null;});
addEventListener('keydown',e=>{if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight',' '].includes(e.key))e.preventDefault();if(mode==='reveal')beginWalk();if(!e.repeat&&e.key===' ')jump();if(!e.repeat&&e.key.toLowerCase()==='e')contextAction();keys.add(e.key.toLowerCase());});addEventListener('keyup',e=>keys.delete(e.key.toLowerCase()));addEventListener('blur',clearInput);document.addEventListener('visibilitychange',()=>{clearInput();clock.getDelta();ambience.visibility(document.hidden);});
renderer.domElement.addEventListener('webglcontextlost',e=>{e.preventDefault();$('error').hidden=false;});
function updateCamera(dt){const up=activeUp();camera.up.copy(up);const eye=player.position.clone().addScaledVector(up,swimming?2.5:2.4);const view=forward.clone().multiplyScalar(Math.cos(pitch)).addScaledVector(up,Math.sin(pitch)).normalize();const offset=Math.max(1.1,(riding?12:9)*(1-Math.max(0,pitch)/1.65));desired.copy(eye).addScaledVector(forward,-offset).addScaledVector(up,riding?1:.65);const radial=desired.clone().sub(CENTER),r=radial.length();if(r>1){const n=radial.divideScalar(r),max=RADIUS-relief(n)-.8;if(r>max)desired.copy(CENTER).addScaledVector(n,max);}camera.position.lerp(desired,1-Math.exp(-dt*12));viewTarget.copy(camera.position).addScaledVector(view,80);camera.lookAt(viewTarget);camera.fov=T.MathUtils.lerp(camera.fov,riding?65:59,Math.min(1,dt*3));camera.updateProjectionMatrix();const nearSun=player.position.distanceTo(CENTER);solarMat.opacity=nearSun<32?Math.max(.05,(nearSun-15)/17*.95):.95;halo.visible=nearSun>32;}
...
function safeSurface(n,radius){return !surfaceBlocked(n,radius)&&towerAllows(n)&&(!riding||lakeRadius(n)>1.05);}
function moveSurface(side,ahead,dt){const speed=riding?48:swimming?6.5:14,steps=Math.max(1,Math.ceil(speed*dt/.38));for(let i=0;i<steps;i++){let next=advanceFrame(navNormal,forward,side,ahead,speed*dt/steps);next.normal=collisions.resolve(next.normal,riding?1.3:.62,jumpHeight);for(const a of expansion.animals){const min=riding?3:2,dist=next.normal.distanceTo(a.n)*RADIUS;if(dist<min){const away=next.normal.clone().addScaledVector(a.n,-next.normal.dot(a.n)).normalize();next.normal.copy(a.n).multiplyScalar(Math.cos(min/RADIUS)).addScaledVector(away,Math.sin(min/RADIUS)).normalize();}}
 if(!safeSurface(next.normal,riding?1.2:.6)){next=advanceFrame(navNormal,forward,side,0,speed*dt/steps);next.normal=collisions.resolve(next.normal,riding?1.3:.62,jumpHeight);if(!safeSurface(next.normal,riding?1.2:.6)){next=advanceFrame(navNormal,forward,0,ahead,speed*dt/steps);next.normal=collisions.resolve(next.normal,riding?1.3:.62,jumpHeight);}}
 if(safeSurface(next.normal,riding?1.2:.6)){navNormal=next.normal;forward=next.forward.projectOnPlane(navNormal).normalize();}}
}
function move(dt){let side=joy.x+(keys.has('d')||keys.has('arrowright')?1:0)-(keys.has('q')||keys.has('a')||keys.has('arrowleft')?1:0),ahead=-joy.y+(keys.has('z')||keys.has('w')||keys.has('arrowup')?1:0)-(keys.has('s')||keys.has('arrowdown')?1:0);const length=Math.hypot(side,ahead);moving=Math.min(1,length);side/=Math.max(1,length);ahead/=Math.max(1,length);
 ...
 if(!climb){if(length>.06)moveSurface(side,ahead,dt);const depth=lakeDepth(navNormal);swimming=!riding&&depth>1.2&&jumpHeight<.3;const waterHeight=depth>0?depth+.24-Math.min(1.25,depth*.9):0;player.position.copy(surface(navNormal,(riding?1.05:waterHeight)+jumpHeight));if(!riding&&depth===0&&jumpHeight<.05){const local=towerLocal(player.position);if(Math.abs(local.y)<1&&Math.hypot(local.x,local.z)<5.8&&expansion.lift.height<.1){climb={x:local.x,z:local.z,y:0,onPlatform:true};forward.projectOnPlane(towerUp).normalize();}}}
 if(length>.06){const up=activeUp(),face=forward.clone().multiplyScalar(ahead).addScaledVector(forward.clone().cross(up).normalize(),side).normalize();const basis=new T.Matrix4().makeBasis(up.clone().cross(face).normalize(),up,face);const q=new T.Quaternion().setFromRotationMatrix(basis);if(swimming)q.multiply(new T.Quaternion().setFromAxisAngle(new T.Vector3(1,0,0),.55));player.quaternion.slerp(q,Math.min(1,dt*14));walk+=dt*(swimming?5:11)*moving;}
 ...
 if(riding){expansion.bike.position.copy(surface(navNormal,1+Math.sin(clock.elapsedTime*3)*.035));expansion.bike.quaternion.copy(player.quaternion);}
 ...
 const p=chart(navNormal);lakeWater.uniforms.swimmer.value.set(p.x,p.z);lakeWater.uniforms.wake.value=T.MathUtils.lerp(lakeWater.uniforms.wake.value,swimming?.4+moving*.6:0,Math.min(1,dt*4));updateCamera(dt);updateHud();}
