...
else if(mode==='tunnel'){const u=Math.min(1,(t-startTime)/6),s=u*u*(3-2*u);const tunnelQ=orientation(normalAt(0,118));camera.position.copy(new T.Vector3(0,4,35-s*73).applyQuaternion(tunnelQ).add(surfacePoint(0,118)));camera.up.copy(normalAt(0,118).negate());const target=surfacePoint(0,45,15);camera.lookAt(target);if(u>=1)beginReveal(t);}
else if(mode==='reveal'){const u=t-startTime;const smooth=v=>{v=T.MathUtils.clamp(v,0,1);return v*v*(3-2*v);};pitch=u<3?.12+1.26*smooth(u/3):u<4.2?1.38:1.38-1.26*smooth((u-4.2)/2.8);updateCamera(dt);if(u>=7)beginWalk();}else move(dt);
if(world.visible){updateSunEffects(t);animatePatapon(t,dt);ambience.update(t,player.position.distanceTo(surfacePoint(-29,-15)),player.position.distanceTo(ship.position),player.position.distanceTo(CENTER),mode==='walk'||mode==='reveal');ripples.forEach(r=>{const s=1+((t*.35+r.userData.phase)%3);r.scale.set(s*1.5,s,1);r.material.opacity=.15*(1-(s-1)/3);});waterMat.color.setHSL(.485+Math.sin(t*.3)*.008,.39,.5);}
