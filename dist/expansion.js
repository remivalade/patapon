...
export function buildExpansion({world,mesh,mat,box,ball,cyl,beam,rand,towerGroup}){
 ...
 for(let i=0;i<=N;i++){const n=roadNormal(i/N*Math.PI*2);for(const sign of [-1,1]){const edge=n.clone().multiplyScalar(Math.cos(5.2/RADIUS)).addScaledVector(ROAD_AXIS,Math.sin(sign*5.2/RADIUS));positions.push(...surface(edge,roadOffset(edge)+.16).toArray());}}
 ...
 for(let i=0;i<240;i++){const t=i/240*Math.PI*2,n=roadNormal(t),f=roadNormal(t+.001).sub(n).normalize().projectOnPlane(n).normalize(),up=n.clone().negate();dummy.position.copy(surface(n,roadOffset(n)+.19));dummy.quaternion.setFromRotationMatrix(new T.Matrix4().makeBasis(up.clone().cross(f).normalize(),up,f));dummy.scale.set(1,1,1);dummy.updateMatrix();markings.setMatrixAt(i,dummy.matrix);}world.add(markings);
 ...
 function updateAnimals(t,dt,playerPosition){for(const a of animals){if(a.ridden)continue;const grazing=a.freeRoam||Math.sin(t*.16+a.index*2)>.05;if(!grazing){a.angle+=dt*.15*Math.sin(t*.17+a.index);const nx=a.x+Math.sin(a.angle)*dt*.55,nz=a.z+Math.cos(a.angle)*dt*.55;if(Math.hypot(nx,nz)<43&&roadDistance(meadowNormal(nx,nz))>9){a.x=nx;a.z=nz;}else a.angle+=dt*1.4;}if(!a.freeRoam)a.n=meadowNormal(a.x,a.z);a.root.position.copy(surface(a.n,roadOffset(a.n)+Math.max(0,lakeDepth(a.n)+.24-.6)*(roadOffset(a.n)===0?1:0)));a.root.quaternion.copy(orientation(a.n)).multiply(new T.Quaternion().setFromAxisAngle(new T.Vector3(0,1,0),a.angle));a.head.rotation.x=a.reaction>t?-.18:grazing?.45+.08*Math.sin(t*.8):0;a.head.rotation.y=a.reaction>t?.15*Math.sin(t*5):0;if(playerPosition.distanceTo(a.root.position)<12){a.root.updateWorldMatrix(true,false);const look=a.root.worldToLocal(playerPosition.clone());a.head.rotation.y=T.MathUtils.clamp(Math.atan2(look.x,look.z),-.7,.7);a.head.rotation.x=-.05;}a.legs.forEach((leg,i)=>leg.rotation.x=grazing?0:Math.sin(t*3+i%2*Math.PI)*.18);}}
